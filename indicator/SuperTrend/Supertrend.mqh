...
namespace SuperTrend {
    ...
    class Supertrend : public TwoLineIndicator {
        ...
        float factor;
        int direction[2];
        double Open[SERIES_SIZE];
        double Close[SERIES_SIZE];
        double High[SERIES_SIZE];
        double Low[SERIES_SIZE];
    ...
    Supertrend::Supertrend(int dataPeriod, float high, float mid, float low) : cluster(datePeriod, high, mid, low){
        ArrayInitialize(supertrend, 0);
